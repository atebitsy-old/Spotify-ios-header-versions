//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "VISREFTopAndBodyContentView.h"

@interface _TtC35PodcastCreatorEntityPageFeatureImpl27CreatorHeaderForegroundView : VISREFTopAndBodyContentView
{
    // Error parsing type: , name: theme
    // Error parsing type: , name: $__lazy_storage_$_imageView
    // Error parsing type: , name: $__lazy_storage_$_metaLabel
    // Error parsing type: , name: $__lazy_storage_$_followButton
    // Error parsing type: , name: $__lazy_storage_$_descriptionLabel
}

- (void).cxx_destruct;
- (id)initWithNaturalHeight:(double)arg1;
- (id)initWithCoder:(id)arg1;

@end

